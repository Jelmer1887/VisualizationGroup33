...
from bokeh.models import ColorBar, LinearColorMapper, HoverTool, BoxSelectTool, CustomJSHover, BoxZoomTool, ResetTool, \
    WheelZoomTool, PanTool, Range1d, DataRange1d, glyph, Row
...
dfSelection = df[SELECTION].copy()         # copy selection from main dataframe

dfPositive = dfSelection[dfSelection['SARS-Cov-2 exam result'] == "positive"]     # filter positive values
dfNegative = dfSelection[dfSelection['SARS-Cov-2 exam result'] == "negative"]     # filter negative values
...
data = {
    "plotname" : bloodGasAnalysis
}

editableData = ColumnDataSource(data)
...
for x, variableName in enumerate(editableData.data['plotname']):                                # create subplots for every bloodtype
    ...
    if count != 0:                               # a seperate plot is made for the first entry
        ...
        scatter = figure(                                   # plots for all the other blood values
            title=variableName,                                    # ... these are the same but they copy their axis from the first one
            plot_width=400,
            plot_height=300,
            y_range=(-4, 4),
            tools="save, pan, reset, wheel_zoom, box_select",
    ...
    p = scatter.square(x=jitter("Patient age quantile", 0.5), y=variableName, size=4, color=colorPositive, alpha=0.5,
                       source=sourcePos, muted_alpha=0.1)
    n = scatter.circle(x=jitter("Patient age quantile", 0.5), y=variableName, size=4, color=colorNegative, alpha=0.5,
...
# Dropdown menu GUI
glyphs = ["Blood Categorie", "Blood Bas Analysis", "Enzym & Minerals & Proteine", "Other Tests"]
...
# GUI Left column
controls = [select, spinner, toggle, multi_choice, but]
